const BlogData = [
  {
    id: 1,

    images: [
      '../../img/room-details/1.png',
      '../../img/room-details/2.png',
      '../../img/room-details/3.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 130 },
      { name: 'Good', value: 65 },
      { name: 'Fine', value: 65 },
      { name: 'Terrible', value: 0 },
    ],

    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 2,
    images: [
      '../../img/room-details/4.png',
      '../../img/room-details/4.png',
      '../../img/room-details/4.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 50 },
      { name: 'Good', value: 100 },
      { name: 'Fine', value: 25 },
      { name: 'Terrible', value: 5 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 3,
    images: [
      '../../img/room-details/5.png',
      '../../img/room-details/5.png',
      '../../img/room-details/5.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 50 },
      { name: 'Good', value: 65 },
      { name: 'Fine', value: 100 },
      { name: 'Terrible', value: 10 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 4,
    images: [
      '../../img/room-details/6.png',
      '../../img/room-details/6.png',
      '../../img/room-details/6.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 130 },
      { name: 'Good', value: 70 },
      { name: 'Fine', value: 65 },
      { name: 'Terrible', value: 10 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 5,
    date: 'January 10, 2022',
    images: [
      '../../img/room-details/7.png',
      '../../img/room-details/7.png',
      '../../img/room-details/7.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 100 },
      { name: 'Good', value: 100 },
      { name: 'Fine', value: 65 },
      { name: 'Terrible', value: 5 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 6,
    images: [
      '../../img/room-details/8.png',
      '../../img/room-details/8.png',
      '../../img/room-details/8.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 70 },
      { name: 'Good', value: 70 },
      { name: 'Fine', value: 60 },
      { name: 'Terrible', value: 20 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 7,
    images: [
      '../../img/room-details/9.png',
      '../../img/room-details/9.png',
      '../../img/room-details/9.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 130 },
      { name: 'Good', value: 50 },
      { name: 'Fine', value: 40 },
      { name: 'Terrible', value: 0 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 8,
    images: [
      '../../img/room-details/10.png',
      '../../img/room-details/10.png',
      '../../img/room-details/10.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 60 },
      { name: 'Good', value: 80 },
      { name: 'Fine', value: 65 },
      { name: 'Terrible', value: 5 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 9,
    images: [
      '../../img/room-details/11.png',
      '../../img/room-details/11.png',
      '../../img/room-details/11.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 30 },
      { name: 'Good', value: 70 },
      { name: 'Fine', value: 60 },
      { name: 'Terrible', value: 15 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 10,
    images: [
      '../../img/room-details/12.png',
      '../../img/room-details/12.png',
      '../../img/room-details/12.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 1200 },
      { name: 'Good', value: 70 },
      { name: 'Fine', value: 50 },
      { name: 'Terrible', value: 0 },
    ],
  },
  {
    id: 11,
    images: [
      '../../img/room-details/13.png',
      '../../img/room-details/13.png',
      '../../img/room-details/13.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 50 },
      { name: 'Good', value: 65 },
      { name: 'Fine', value: 65 },
      { name: 'Terrible', value: 20 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
  {
    id: 12,
    images: [
      '../../img/room-details/14.png',
      '../../img/room-details/14.png',
      '../../img/room-details/14.png',
    ],
    info: [
      'Noise-absorbing walls',
      'A window in each of the rooms',
      'The room with a fireplace',
    ],
    votes: [
      { name: 'Excellent', value: 50 },
      { name: 'Good', value: 30 },
      { name: 'Fine', value: 30 },
      { name: 'Terrible', value: 10 },
    ],
    para: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum.',
    desc: 'Lorem ipsum dolor sit amet consectetur ipiscing elit amet consectetur piscing elit consectetur adipiscing elit sed et eletum. orem ipsum dolor sit amet consectetur adipiscing elit amet consectetur piscing elit amet consectetur adipiscing elit sed et eletum nulla eu placerat felis etiam tincidunt orci lacus id varius dolor fermum sit amet.',
  },
];

// console.log(BlogData[0].data[0].value);
export default BlogData;
